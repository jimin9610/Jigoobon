...
<table width=850px border=0 align=center>
	...
	<tr>
		<td height=50px style="border-bottom: 1px solid #c1edff"><b>글쓴이 : </b><%=nick%></td>
		<td align=right style="border-bottom: 1px solid #c1edff;line-height:150%"><b>작성날짜 : </b><%=wrdate %><br><b>조회수 : </b><%=ref %> </td>
	</tr>
	
	<tr>
		<td height=300 valign=top colspan=2 style="padding:20px;border-bottom: 1px solid #99e0ff">
		<%if(!file1.equals("")){%>
		<img width=500px src="/upload/<%=file1%>">
		<br>
		<%}%>
		<%=comment %>
		</td>
...
<!-- 한줄 댓글 -->
<form action="comment_insert.jsp?code=<%=code%>&uid=<%=uid%>" method="post">
...
<table width=850px border=0 align=center style="border-bottom: 1px solid #0086bf">
	...
	<tr>
		...
		<td align=center><input name="view_comment" style="width:500px;height:50px;border:1px solid #a2a2a2;border-radius:5px;"></td>
